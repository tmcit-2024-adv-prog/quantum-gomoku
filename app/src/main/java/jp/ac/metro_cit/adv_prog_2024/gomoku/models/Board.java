...
public class Board {
  ...
  public boolean checkWinner(Vector2D pos) {
    ...
    return (countStones(pos, 1, 0, currentColor) + countStones(pos, -1, 0, currentColor) >= 4)
        || // 横
        (countStones(pos, 0, 1, currentColor) + countStones(pos, 0, -1, currentColor) >= 4)
        || // 縦
        (countStones(pos, 1, 1, currentColor) + countStones(pos, -1, -1, currentColor) >= 4)
        || // 斜め（右下/左上）
        (countStones(pos, 1, -1, currentColor) + countStones(pos, -1, 1, currentColor)
            >= 4); // 斜め（右上/左下）
  }
